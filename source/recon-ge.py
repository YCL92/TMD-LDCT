...
def rebinNFilter(data_dir, save_dir, proj_md, flt_kernel, device):
    ...
    # interpolation grids
    fs_grid = t.zeros((1, n_rebin_chnls, 1, 2), device=device)
    fs_grid[0, :, 0, 0] = proj_md["dr_idx"] / proj_md["proj_offset"]
    fs_grid[0, :, 0, 1] = 1 - 2 * proj_md["p_idx"] / (proj_md["n_chnls"] - 1)

    # temporary buffers
    raw_data = t.zeros((1, proj_md["n_rows"], proj_md["n_chnls"], proj_offset_full + 1), device=device)
    ...
    for r_idx in tqdm(range(n_projs), desc="Rebin", total=n_projs):
        # grab data
        raw_data = t.roll(raw_data, -1, dims=-1)

        file = dcmread(os.path.join(data_dir, "%06d.dcm" % r_idx))
        ...
        # rebining
        rebin_data[0, :, :] = grid_sample(
            raw_data, fs_grid, mode="bilinear", padding_mode="border", align_corners=True
        ).squeeze()
        ...
        # extra metadata
        file = dcmread(os.path.join(data_dir, "%06d.dcm" % (r_idx - proj_md["proj_offset"])))
